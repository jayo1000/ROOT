...
<h2 style="font-size: 40px; font: bold;">대출설계</h2>
<h6 style="margin-top: 17px; color:#999999">고객님을 위한 미리보는 대출설계 시뮬레이션</h6>
</div>
<form name="Consultation" class="form-horizontal" method="post" id="panel"> <!-- action은 아직 추가안함 -->
...
<div class="sub_contents">
		...
		<p class="mg_b30"><img src="images/recommand.gif" alt="대출설계 시뮬레이션 이미지"/></p>

		<form name="frmNext" id="frmNext" method="post" action="">
		<input type="hidden" name="hidLoanType" value="888" />
		<input type="hidden" name="hidMedia" value="2" />
		<input type="hidden" name="hidKeyWord" value="119머니" />
		<input type="hidden" name="hidJobcode" value="1" />
		<input type="hidden" name="returnUrl" value="http://www.xn--119-db3mp11a.com/Procedure/FrmProcedureRecommend4.asp" />
		<input type="hidden" name="hidSiteSeq" value="193" />

		<table border="0" cellspacing="0" cellpadding="0" class="boardType1">
		<colgroup>
			<col width="160"/>
			<col width="340"/>
			<col width="160"/>
		...
		<tbody>
			<tr style="border-top:5px solid black;">
				<th style="background-color: #e6e6e6; padding-left: 30px;"s>성별</th>
				<td style="padding-left: 20px;"><input name="hidSex" type="radio" id="gender1" value="1" checked="checked"/>
					<label for="gender1">남자</label>&nbsp;&nbsp;
					<input name="hidSex" type="radio" value="2" id="gender2" />
					<label for="gender2">여자</label></td>

				<th style="background-color: #e6e6e6; padding-left: 30px;">나이</th>
				<td style="padding-left: 20px;"><select name="cYear" id="ddlYear" style="width:100px; height:26px;">
					</select>
					년 / 만
					<input type="text" name="iAge" id="txtAge" style="width:50px;height:24px;" size="4" maxlength="2" />
					세</td>
			</tr>
			<tr style="border-top:1px solid black;">
				<th style="background-color: #e6e6e6; padding-left: 30px;">결혼여부</th>
				<td style="padding-left: 20px;"><input name="tiMarry" type="radio" value="1" />
					기혼
					&nbsp;&nbsp;<input name="tiMarry" type="radio" value="2" checked="checked" />
					미혼</td>

				<th style="background-color: #e6e6e6; padding-left: 30px;">대출금액</th>
				<td style="padding-left: 20px;"><select id="iReqgumek" name="iReqgumek" class="text_12_01" style="width:100px; height:26px;">
			...
			<tr style="border-top:1px solid black;">
				<th style="background-color: #e6e6e6; padding-left: 30px;">대출금리</th>
				<td style="padding-left: 20px;">
					<input class="text_12_01" type="text" id="iGumri" name="iGumri" size="10" maxlength="20" value="10" style="width:78px; line-height:24px; height:24px; ime-mode:inactive;" />	% (연단뒤)
				</td>
				<th style="background-color: #e6e6e6; padding-left: 30px;">대출기간</th>
				<td style="padding-left: 20px;">
					<select id="iReqgigan" name="iReqgigan" class="text_12_01" style="width:100px; height:26px;">
			...
			<tr style="border-top:1px solid black;">
				<th style="background-color: #e6e6e6; padding-left: 30px;">연락처</th>
				<td style="padding-left: 20px;">
					<select name="ddlMobilePhone" id="ddlMobilePhone2" style="width:60px; height:26px; line-height:26px;">	</select>
					-
					<input type="text" name="txtMobilePhone2" id="txtMobilePhone2" class="T-box" style="width:50px;height:24px;" size="4" maxlength="4" />
					-
					<input type="text" name="txtMobilePhone3" id="txtMobilePhone3" class="T-box" style="width:50px;height:24px;" size="4" maxlength="4" /></td>

				<th style="background-color: #e6e6e6; padding-left: 30px;">직업</th>
				<td style="padding-left: 20px;"><select id="tiJob" name="tiJob" class="text_12_01" style="width:100px; height:26px;">
						<option value="1">직장인</option>
						<option value="2">사업자</option>
						<option value="3">주부</option>
						<option value="4">프리랜서</option>
						<option value="5">무직자</option>
					</select></td>
			</tr>
			<tr style="border-top:1px solid black;">
				<th style="background-color: #e6e6e6; padding-left: 30px;">상환방법</th>
				<td colspan="3" style="padding-left: 20px;">
					<span>
					<input id="tiCalType1" name="tiCalType" type="radio" value="1" checked="checked" />
					<label for="tiCalType1" style="cursor:pointer;">원금자유상환</label>
					</span>

					<span>
					<input id="tiCalType2" name="tiCalType" type="radio" value="2" />
					<label for="tiCalType2" style="cursor:pointer;">원리금 분등균할상환</label>
					</span>
				</td>
			</tr>
			<tr style="border-top:1px solid black;"><td></td><td></td></tr>
			<tr>
			<td><div style="margin-top: 30px; border-top:3px solid black;"></div></td>
			<td><div style="margin-top: 30px; border-top:3px solid black;"></div></td>
			<td><div style="margin-top: 30px; border-top:3px solid black;"></div></td>
			<td><div style="margin-top: 30px; border-top:3px solid black;"></div></td>
			</tr>
		...
		<!--개인(신용)정보 수집·이용 동의 [필수] 시작-->
		<div class="boardType2 mg_t20" style="margin-top: 10px;">
			...
			<tbody>
				<tr>
					<td colspan="2">
						<!-- 개인정보취급방침 시작 -->
						<textarea cols="50" rows="13" readonly style="width:960px; height:200px; border:1px solid #d3d3d3; padding:15px; font-family:NanumGothic, 나눔고딕; font-size:14px; color:#666; line-height:26px;">
						1. (필수) 개인신용정보 조회 동의서
...
(3) 조회동의 효력 기간
귀하가 상기 동의서를 제출한 시점부터 당해 대출계약의 효력이 종료하는 시점까지 상기 동의의 효력이 유지됩니다. 다만, 귀하가 신청한 대출이 본 119머니대부중개에 의해 거절된 경우에는 그 시점부터 동의의 효력은 소멸합니다.
※ 본 119머니대부중개가 신용조회회사를 통하여 귀하의 신용정보를 조회한 기록은 타 금융기관 등에 제공될 수 있으며, 귀하의 신용등급이 하락할 수 있음을 알려드립니다. 단, 2011년 10월 4일부터 귀하의 신용정보를 조회한 기록은 신용평가 목적으로 타 금융기관 등에 제공되지 않으며, 무등급자의 신용평가 목적 이외에는 신용등급 산정에 반영되고 있지 않습니다.
...
(2) 제휴회사에 개인신용정보 제공
■ 제공 대상 기관
① ㈜바로크레디트대부, 119머니대부㈜ ㈜태강대부, 에이앤피파이낸셜대부㈜, 산와대부주식회사등 각 지방자치단체에 등록된 대부업체
② 한국대부금융협회
③ ㈜ 리치앤리치대부중개
...
개인정보취급(처리)방침
119머니대부중개(119머니.com)와 (주)리치앤리치대부중개는 신용정보의 이용 및 보호에 관한 법률, 정보통신망 이용촉진 및 정보보호 등에 관한 법률, 개인정보보호법, 통신비밀보호법 등 관련 법령에 따라 이용자의 개인정보 보호 및 권익을 보호하고 개인정보와 관련한 이용자의 고충을 원활하게 처리할 수 있도록 다음과 같은 취급(처리)방침(이하 “방침”)을 두고 있습니다. 119머니대부중개와 ㈜리치앤리치대부중개는 본 방침을 개정하는 경우 웹사이트 공지사항(또는 개별공지)을 통하여 공지할 것입니다.

○ 본 방침은 2014년 4월 1일부터 시행됩니다.
1). 개인정보의 처리 목적
119머니대부중개와 ㈜리치앤리치대부중개는 개인정보를 다음의 목적을 위해 처리합니다. 처리한 개인정보는 다음의 목적 이외의 용도로는 사용되지 않으며 이용 목적이 변경될 시에는 사전동의를 구할 예정입니다.
...
3). 개인정보의 처리 및 보유 기간
① 119머니대부중개와 ㈜리치앤리치대부중개는 법령에 따른 개인정보 보유•이용기간 또는 고객으로부터 개인정보를 수집 시에 동의 받은 개인정보 보유•이용기간 내에서 처리, 보유합니다.
...
119머니대부중개는 법령의 규정에 의한 경우 또는 아래 각 항에서 보는 바와 같이 고객으로부터 별도의 동의를 얻은 경우에만 개인정보를 제3자에게 제공합니다.
...
※ 정보 보유•이용기간 
제공 동의일로부터 당해 대출계약의 효력이 종료하는 시점까지 상기 동의의 효력이 유지됩니다. 다만, 귀하가 신청한 대출이 119머니대부중개에 의해 거절된 경우에는 그 시점부터 동의의 효력은 소멸합니다. 
...
① 수탁업체
119머니대부중개와 계약을 맺은 업체, ㈜리치앤리치대부중개, nice신용평가정보㈜, 코리아크레딧뷰로(주)

② 위탁업무 내용
대부중개(개인대출모집, 고객특성분석, 신용관련통계분석모형개발, 시장환경분석, 대출정보 복원 등)
6). 정보주체의 권리, 의무 및 그 행사방법
① 정보주체는 개인정보보호법에 따라 119머니대부중개에 대해 다음과 같은 개인정보 관련 권리를 행사할 수 있습니다.
 1) 개인정보 열람요구
 2) 오류 등이 있을 경우 정정 요구
 3) 삭제요구
 4) 처리정지 요구

② 제1항에 따른 권리 행사는 119머니대부중개에 대해 관련 법령에 규정된 절차에 따라 서면, 모사전송(FAX) 등을 통하여 하실 수 있으며 119머니대부중개는 이에 대해 지체 없이 조치하겠습니다. 

③ 정보주체가 개인정보의 오류 등에 대한 정정 또는 삭제를 요구한 경우에는 119머니대부중개는 정정 또는 삭제를 완료할 때까지 당해 개인정보를 이용하거나 제공하지 않습니다.

④ 제1항에 따른 권리 행사는 다음과 같이 진행됩니다.

1) 개인정보에 대한 열람 청구권 
정보주체는 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체가 처리하는 정보주체의 개인정보에 대한 열람을 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체에 요구할 수 있습니다. 이 경우 정보주체는 행정안전부령으로 정하는 바에 따라 다음 각 호의 사항 중 열람하려는 사항을 표시한 개인정보요구서를 회사에 제출하여야 하며 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체는 개인정보요구서를 수령한 날로부터 10일 이내에 정보주체에게 개인정보를 열람할 수 있도록 조치하겠습니다. 단, 법률에 따라 열람이 금지되거나 다른 사람의 생명, 신체를 해할 우려가 있는 경우 등의 사유로 열람이 제한, 연기 및 거절될 수 있습니다.

2) 개인정보의 정정ㆍ삭제 요구권 
개인정보를 열람한 정보주체는 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체에 그 개인정보의 정정 또는 삭제를 요구할 수 있습니다. (단, 다른 법령에서 그 개인정보가 수집 대상으로 명시되어 있는 경우에는 그 삭제를 요구할 수 없음).이 경우 정보주체는 행정안전부령으로 정하는 개인정보 정정ㆍ삭제 요구서를 회사에 제출하여야 하며 개인정보 정정ㆍ삭제요구서를 수령한 날로부터 10일 이내에 해당 개인정보를 정정ㆍ삭제하겠습니다.
3) 신용정보 처리정지 요구권 
정보주체는 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체에 대하여 개인정보 처리의 정지를 요구할 수 있습니다. 이 경우 정보주체는 행정안전부령으로 정하는 개인정보 처리정지 요구서를 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체에 제출하여야 하며 개인정보 처리정지 요구서를 수령한 날로부터 10일 이내에 해당 개인정보 처리를 정지하겠습니다. 단, 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체와 거래 중이며 해당 거래를 유지하고자 하는 정보주체는 개인정보 처리정지 시 거래관계를 유지할 수 없으므로 개인정보보호법 제37조 2항 4조에 따라 119머니대부중개 또는 대출중개를 위하여 계약을 맺은 업체는 개인정보 처리정지 요구를 거절할 수 있습니다.

⑤ 개인정보 요구서(열람/정정,삭제/처리정지) 및 이의제기 양식은 119머니.com사이트의 메인 페이지 에 있는 [고객의 권리] 메뉴에서 다운받으실 수 있으며 아래와 같은 방식으로 발송해주십시오.
1) 팩스 : 032-421-0884
...
7). 개인정보의 파기
119머니대부중개는 원칙적으로 개인정보 처리목적이 달성된 경우에는 지체없이 해당 개인정보를 파기합니다. 파기의 절차, 기한 및 방법은 다음과 같습니다.
...
8). 개인정보의 안전성 확보 조치
119머니대부중개는 개인정보를 취급함에 있어 개인정보가 분실, 도난, 누출, 변조 또는 훼손되지 않도록 안전성 확보를 위하여 다음과 같은 기술적•관리적 및 물리적 대책을 강구하고 있습니다.
...
⑤ 해킹 등에 대비한 기술적 대책
119머니대부중개는 해킹이나 컴퓨터 바이러스 등에 의한 개인정보 유출 및 훼손을 막기 위하여 보안프로그램을 설치하고 주기적인 갱신•점검을 하며 외부로부터 접근이 통제된 구역에 시스템을 설치하고 기술적/물리적으로 감시 및 차단하고 있습니다. 
...
119머니대부중개는 귀하의 정보를 수시로 저장하고 찾아내는 '쿠키(cookie)' 등을 운용합니다. 쿠키란 어떤 웹사이트를 운영하는데 이용되는 서버가 귀하의 브라우저에 보내는 아주 작은 텍스트 파일로서 귀하의 컴퓨터 하드디스크에 저장됩니다. 회사는 다음과 같은 목적을 위해 쿠키를 사용합니다. 
...
10). 개인정보 관리(보호)책임자 
① 119머니대부중개는 개인정보 처리에 관한 업무를 총괄해서 책임지고 개인정보 처리와 관련한 고객의 불만처리 및 피해구제 업무 등을 수행하기 위하여 아래와 같이 개인정보 관리(보호)책임자를 지정하고 있습니다."
...
② 고객께서는 119머니대부중개의 서비스(또는 사업)을 이용하시면서 발생한 모든 개인정보 보호 관련 문의, 불만처리, 피해구제 등에 관한 사항을 개인정보 보호책임자 및 담당부서로 문의하실 수 있습니다. 
119머니대부중개는 고객의 문의에 대해 지체 없이 답변 및 처리해드릴 것입니다. 개인정보에 관한 권익을 침해 당한 경우 개인정보 침해신고센터, 대검찰청, 사이버경찰청 등으로 문의하실 수 있습니다. 
...
119머니대부중개는 법정대리인의 동의가 필요한 만14세 미만 아동의 가입은 받고 있지 않습니다.
12). 개인정보 취급(처리)방침 변경 
...
제 1 장 총칙
제1조(목적)
본 약관은 119머니대부중개 (이하 ‘회사’라고 칭함)가 운영하는 웹사이트(이하 ‘사이트’라고 한다)에서 제공하는 인터넷 관련 서비스를 이용함에 있어, 회사와 이용자의 권리, 의무 및 책임사항을 규정함을 목적으로 합니다.
...
<!--왼쪽 약관동의 끝-->
		...
		<a href="#"><img src="images/main-dacul.png"></a>
		</div>
		<div align="center">
<img src="images/main-down.png" style="margin-bottom: 10px;">
</div>
</form>
<!-- 스크립트 모음 -->
